...
判斷模式：
  有設 DEEPGRAM_API_KEY → real 模式（sounddevice + webrtcvad + Deepgram）← TODO 硬體到位後實作
  沒設                   → mock 模式（startListening 後 2s 發「你好」假 speech）
"""
...
class AudioModule:
    ...
    async def start(self) -> None:
        if self.deepgram_key:
            ...
            print("[audio] mock mode — startListening 後 2s 發「你好」假 speech")
